...
class Solicitud_interconsulta_controller extends REST_Controller
{
    public function __construct() {
        parent::__construct();
        $this->load->model('solicitud_interconsulta_model', 'mymodel');
    }
    ...
    public function index_get($id = NULL) {
        ...
        if (is_null($id))
        {
            ...
            if (!is_null($query))
            {
                $this->response(array('solicitud_interconsulta' => $query), 200);
            }
            else{
