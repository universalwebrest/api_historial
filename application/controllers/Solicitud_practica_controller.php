...
class Solicitud_practica_controller extends REST_Controller
{
    public function __construct() {
        parent::__construct();
        $this->load->model('solicitud_practica_model', 'mymodel');
    }
    ...
    public function index_get($id = NULL) {
        ...
        if (is_null($id))
        {
            ...
            if (!is_null($query))
            {
                $this->response(array('solicitud_practica' => $query), 200);
            }
            else{
