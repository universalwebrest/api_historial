...
class Tratamiento_actual_controller extends REST_Controller
{
    public function __construct() {
        parent::__construct();
        $this->load->model('tratamiento_actual_model', 'mymodel');
    }
    ...
    public function index_get($id = NULL) {
        ...
        if (is_null($id))
        {
            ...
            if (!is_null($query))
            {
                $this->response(array('tratamiento_actual' => $query), 200);
            }
            else{
