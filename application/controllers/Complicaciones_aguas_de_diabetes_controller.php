...
class Complicaciones_agudas_de_diabetes_controller extends REST_Controller
{
    public function __construct() {
        parent::__construct();
        $this->load->model('complicaciones_agudas_de_diabetes_model', 'mymodel');
    }
    ...
    public function index_get($id = NULL) {
        ...
        if (is_null($id))
        {
            ...
            if (!is_null($query))
            {
                $this->response(array('complicaciones_agudas_de_diabetes' => $query), 200);
            }
            else{
