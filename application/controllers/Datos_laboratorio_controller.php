...
class Datos_laboratorio_controller extends REST_Controller
{
    public function __construct() {
        parent::__construct();
        $this->load->model('datos_laboratorio_model', 'mymodel');
    }
    ...
    public function index_get($id = NULL) {
        ...
        if (is_null($id))
        {
            ...
            if (!is_null($query))
            {
                $this->response(array('datos_laboratorio' => $query), 200);
            }
            else{
